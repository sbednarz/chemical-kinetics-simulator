...
//---------------------------------------------------------------------------
//    Definition of function ProbCalc for reversible mass diffusion
...
void mass_diffusion :: ProbCalc()

{

     FLOAT64 		q, np, T, P, Order;
...

     {
	     ...
	     // calculate mass transfer probabilities
		...
		while ( ReactantCounter < ProcessData.numSpeciesInRateLaw )
		{
		  ...
		  // add in dependence on temperature and pressure if the phase of the source compartment is
		  // GAS - assumes that the temperature dependence was not put into the diffusion
		  // coefficient parameters

		  P = ((transfer_path *)pProcessArea)->GetPressure();

		  if ( P ) 	// transfer path links 2 gas phase volume elements
		  {
			if ( (((transfer_path *)pProcessArea)->IsConstP() == FALSE) ||
				(((transfer_path *)pProcessArea)->IsConstT() == FALSE) )
			  {
				q *= ( pow ( P,1.50) ) / T;
			  }
		  }
